...
class Board {
    ...
    PieceColor getWinner() {
        if (numPieces(RED) + numPieces(BLUE) == _totalOpen) {
    ...
    boolean canMove(PieceColor who) {
        ...
        for (char i = 'a'; i <= 'g'; i++) {
            for (char j = '1'; j <= '7'; j++) {
                if (get(index(i, j)) == who) {
                    for (int col = -2; col <= 2; col++) {
                        for (int row = -2; row <= 2; row++) {
                            int target = neighbor(index(i, j), col, row);
                            if (get(target) == EMPTY) {
                                return true;
                            }
                        }
    ...
    void makeMove(Move move) {
        ...
        for (int col = -1; col <= 1; col++) {
            for (int row = -1; row <= 1; row++) {
                int adjacentIndex = neighbor(move.toIndex(), col, row);
                if (get(adjacentIndex) == _whoseMove.opposite()) {
                    set(adjacentIndex, _whoseMove);
                    incrPieces(_whoseMove, 1);
    ...
    boolean legalBlock(char c, char r) {
        return _board[index(c, r)] == EMPTY && _numMoves == 0;
    }
    ...
    void setBlock(char c, char r) {
        ...
        char colReflect = 'a';
        char[] cols = new char[] {'a', 'b', 'c', 'd', 'e', 'f', 'g'};
        for (int i = 0; i < cols.length; i += 1) {
            if (cols[i] == c) {
                colReflect = cols[cols.length - i - 1];
            }
        }
        char rowReflect = '1';
        ...
        for (int i = 0; i < rows.length; i += 1) {
            if (rows[i] == r) {
                rowReflect = rows[rows.length - i - 1];
            }
        }
